...
if [ $# -lt 1 ]
  then
   ...
   # fastqc -t $threads \
   # $fastqs_path/"$ID"/"$ID"_*"$forward_suff" \
   # $fastqs_path/"$ID"/"$ID"_*"$reverse_suff" \
   ...
   # echo $ID" pre-trim QC done" >> trim_and_QC_log.txt
    ...
    $fastqs_path/"$ID"/"$ID"_*"$forward_suff" $fastqs_path/"$ID"/"$ID"_*"$reverse_suff" \
    -baseout $fastqs_path/"$ID"/"$ID"_trimmed.fq.gz \
    ...
    fastqc -t $threads \
    $fastqs_path/"$ID"/"$ID"_trimmed_1P.fq.gz \
    $fastqs_path/"$ID"/"$ID"_trimmed_2P.fq.gz \
